...
{                                                                   
    ...
    {                                                                   
        ...
        AVLNode<K,V> parent = (AVLNode<K,V>) lastStep.parent;
        AVLNode<K,V> child;
        while ( reduced && parent != null )
        {
            if ( lastStep.isLeftChild )
            	if (parent.getBalance()=='R'){
	            	child = (AVLNode<K,V>) parent.getRight();
	                switch ( child.getBalance() )
	                {
	                    case 'L': 
	                        this.rotateRight2(parent, child,path);
	                        reduced = true;
	                        break;
	                    case 'E': 
	                        this.rotateRight1E(parent, child, path);
	                        reduced = false;
	                        break;
	                    case 'R': 
	                        this.rotateRight1R(parent, child, path);
	                        reduced = true;
	                        break;
	                }
            	}
            	...
            	if (parent.getBalance()=='L'){
	            	child = (AVLNode<K,V>) parent.getLeft();
	                switch ( child.getBalance() )
	                {
	                    case 'L': 
	                        this.rotateLeft1L(parent, child,path);
	                        reduced = true;
	                        break;
	                    case 'E': 
	                        this.rotateLeft1E(parent, child, path);
	                        reduced = false;
	                        break;
	                    case 'R': 
	                        this.rotateLeft2(parent, child, path);
	                        reduced = true;
	                        break;
	                }
            	}
